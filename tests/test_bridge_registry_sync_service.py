...
class BridgeRegistrySyncServiceTests(unittest.TestCase):
    ...
    def test_register_mapping_for_office_file_keeps_pdf_path_empty(self):
        tmp_path = Path(self.id().replace(".", "_"))
        if tmp_path.exists():
            import shutil
        ...
        service.register_mapping(
            task={
                "doc_id": "doc-office",
                "final_md_filename": "doc-office.md",
        ...
        self.assertEqual(captured["json"]["origin_pdf_name"], "员工手册.docx")
        self.assertIsNone(captured["json"]["pdf_abs_path"])
