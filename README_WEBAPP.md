...
这是一个包在当前 MinerU 环境外层的内部知识库文件管理系统，用于在局域网内上传 PDF、Word、Excel 或 ZIP、选择所属知识库及其子目录、保存规范命名后的原始文件，以及提取最终 Markdown。
...
- `data/pdf_store/`：规范命名后的原始文件，文件名是 `<doc_id>.<ext>`
...
- `/tasks`：任务列表
- `/files`：知识库文件管理页
- `/files/{doc_id}`：文件详情页，可生成并打开原始文件受控访问链接
- `/files/{doc_id}`：也会展示 FastGPT 同步状态，并在失败时提供重试入口

上传时必须选择所属知识库，并可继续从该知识库的目录树中选择已有子目录；不选择子目录时默认落到知识库根目录。上传入口支持两种方式：

- 直接上传一个或多个 PDF、`.docx`、`.xlsx`、`.xlsm`
- 上传一个或多个 ZIP；ZIP 内多层目录会挂到所选目标目录之下并继续保留层级，不支持的文件自动跳过

当前继续按单个文件限制大小，默认上限 `200 MB`。ZIP 不额外施加整包业务限额，但其中每个支持文件仍会单独校验。
...
选择某个知识库后，左侧会继续显示该知识库下的子目录树。点击目录时，文件表会展示该目录及其下级子目录中的文件。上传页只选择已有目录，新建、重命名、删除空目录仍在 `/files` 文件管理页完成；目录归属只影响文件中心，不改变 FastGPT 知识库匹配或 Bridge 回填目标。

`/files` 文件表默认每页展示 `20` 条，也可在工具栏切换为 `50 / 100 / 200 / 500` 条。`page_size` 会进入 URL，并在筛选、分页、移动、删除和详情页返回时保留；切换档位时回到第 1 页。当前页面顶部工具区已按“筛选与导出 / 批量操作”分层拆开，避免控件挤在一起。列表区域保持固定高度，显示更多文件时在表格内部滚动，不会把整页继续撑高。导出 Excel 仍按当前筛选范围导出，不受分页大小限制。
...
Word/Excel 不调用 MinerU，会在本地转换成 Markdown：Word 尽量保留段落、列表和表格；Excel 按工作簿生成一个 Markdown，每个非空工作表单独成段。

## 原始文件受控访问链接

文件管理中心可基于 SQLite 中的 `doc_id` 生成短时效签名链接，只允许访问单个原始文件，不开放目录浏览，也不会在 URL 中暴露真实绝对路径。PDF 会以内联方式打开，Office 文件按附件下载。
...
```text
GET /api/files/file-link?doc_id=<doc_id>
```

返回字段包括 `doc_id`、`file_url`、`pdf_url`、`filename`、`file_type`、`expires_in`、`expires_at`。旧接口 `GET /api/files/pdf-link?doc_id=<doc_id>` 继续保留兼容。
...
如果你的 Bridge 和文件中心不在同一台服务器，通常可以保持 `enabled = false`，只配置 `api_base_url`，这样文件中心会在 FastGPT 同步成功后直接把轻映射回填给 Bridge，不再复制 PDF。Word/Excel 第一版不参与本地 PDF manifest 导出，只会在 FastGPT 同步成功后复用 registry 回填。
