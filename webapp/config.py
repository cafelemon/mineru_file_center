...
class Settings:
    ...
    task_workers: int = 1
    mineru_process_timeout_seconds: int = 0
    mineru_stale_process_max_age_seconds: int = 3600
    mineru_stale_process_scan_interval_seconds: int = 600
...
def get_settings() -> Settings:
    ...
    mineru_process_timeout_seconds = _coerce_int(
        os.getenv("MINERU_PROCESS_TIMEOUT_SECONDS")
        or _get_nested(config, "mineru", "process_timeout_seconds", default=0),
        0,
    )
    mineru_stale_process_max_age_seconds = _coerce_int(
        os.getenv("MINERU_STALE_PROCESS_MAX_AGE_SECONDS")
        or _get_nested(
            config,
            "mineru",
            "stale_process_max_age_seconds",
            default=3600,
        ),
        3600,
    )
    mineru_stale_process_scan_interval_seconds = _coerce_int(
        os.getenv("MINERU_STALE_PROCESS_SCAN_INTERVAL_SECONDS")
        or _get_nested(
            config,
            "mineru",
            "stale_process_scan_interval_seconds",
            default=600,
        ),
        600,
    )
    ...
    return Settings(
        ...
        mineru_process_timeout_seconds=max(0, mineru_process_timeout_seconds),
        mineru_stale_process_max_age_seconds=max(0, mineru_stale_process_max_age_seconds),
        mineru_stale_process_scan_interval_seconds=max(0, mineru_stale_process_scan_interval_seconds),
